...
MPM به صورت خودکار این تدابیر امنیتی را در اولین اجرا اعمال می‌کند:

1. **مجوزهای امن فایل** - `.config/` روی 0700، فایل‌ها روی 0600
2. **htaccess. خودکار** - دسترسی وب به همه فایل‌های مخفی را مسدود می‌کند
3. **راه‌اندازی localhost** - کلید API فقط از CLI یا localhost HTTP نشان داده می‌شود
4. **هدرهای امنیتی HTTP** - X-Frame-Options، CSP، X-Content-Type-Options
5. **تولید کلید CSPRNG** - از `random_int()` برای امنیت رمزنگاری استفاده می‌کند

توصیه‌های اضافی:

1. **فقط HTTPS** در تولید فعال کنید
2. **محدود کردن نرخ** در سطح سرور وب
3. **لیست سفید IP** برای دسترسی فقط ادمین
4. **نظارت بر تلاش‌های احراز هویت ناموفق**
5. **به‌روزرسانی‌های امنیتی منظم**

برای nginx، این را به بلوک سرور خود اضافه کنید:
